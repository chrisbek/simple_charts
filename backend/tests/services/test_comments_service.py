...
@pytest.mark.asyncio
async def test_get_all_comment_threads_should_call_thread_repository_list(get_all):
    get_all.return_value = initial_threads

    result = get_all_comment_threads()

    get_all.assert_called_once_with()
    assert result == initial_threads


@patch("repositories.comment_threads_repository.get")
@pytest.mark.asyncio
async def test_get_comment_thread_should_call_thread_repository_get(get):
...
@patch("repositories.comment_threads_repository.get")
@pytest.mark.asyncio
async def test_get_threads_should_throw_if_id_is_not_found(get):
...
async def test_create_thread_should_call_thread_repository_create_if_datapoint_is_new(
        get_by_data_point, repo_create_thread):
    ...
    assert result.chart_data_point == data_point


@patch("repositories.comment_threads_repository.add_comment_to_thread")
@patch("repositories.comment_threads_repository.get_by_data_point")
@pytest.mark.asyncio
async def test_create_thread_should_switch_to_add_comment_if_thread_exists(get_by_data_point, add_comment):
    comment = Comment(user_name="Josh", text="Testing")
    data_point = ChartDataPoint(country=Country.ES, feature=ChartDataFeature.HOTDOG)
...
@patch("repositories.comment_threads_repository.get")
@pytest.mark.asyncio
async def test_respond_to_thread_should_call_add_comment(get, add_comment):
    comment = Comment(user_name="Josh", text="Testing")
...
@patch("repositories.comment_threads_repository.get")
@pytest.mark.asyncio
async def test_respond_to_thread_should_throw_if_thread_does_not_exist(get):
    comment = Comment(user_name="Josh", text="Testing")
    get.return_value = None
