...
from services import data_service, share_service, test_comments_service
...
def get_deployment():
    return list(
        map(
            ...
            test_comments_service.get_all_comment_threads(),
        )
    )
...
def get_deployment(thread_id: str):
    return test_comments_service.get_comment_thread(UUID(thread_id)).to_dict()


@app.post("/chart/comment_threads")
def create_thread(request: CreateThreadRequest):
    return test_comments_service.create_thread(
...
def create_thread(thread_id: str, request: RespondToThreadRequest):
    return test_comments_service.respond_to_thread(
